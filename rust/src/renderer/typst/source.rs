...
impl Renderer {
    ...
    pub(crate) fn build_parameterized_source(scene: &Scene) -> (String, HashMap<Label, String>) {
        ...
        // 3. Rust-managed scene gating. Scenes are flat and mutually exclusive
        //    (nesting is a parse error), so each `#scene(...)` call is wrapped in
        //    a code block that reads `candy:active_scene` and emits its own
        //    `page()` when it is the active scene (or when no scene is selected),
        //    and `none` otherwise. No weird parameters are added to the Typst
        //    `scene` function; the gating is fully generated by Rust.
        ...
        for (k, &sid) in ordered.iter().enumerate() {
            ...
            let wrapper = format!(
                "{{{{ let __a = sys.inputs.at(\"candy:active_scene\", default: 0); if __a == 0 or __a == {sid} {{ {call_inner} }} else {{ none }} }}}}"
            );
    ...
    /// * scene `sid`'s own body context (its `#set` / `#show` / `#let` /
    ///   content), minus its own `#mobject` calls.
    ///
    /// Scenes are flat — nesting is a parse error — so the "keep" set is always
    /// the single scene itself.
    ///
    ...
    pub(crate) fn build_scene_contexts(
        ...
        for s in &scene.scenes {
            let keep: HashSet<usize> = std::iter::once(s.id).collect();
            let mut edits: Vec<(usize, usize, Option<String>)> = Vec::new();
            Self::collect_skipped(src, &node, &keep, &call_map, &mut edits, scene, wrapped);
            let ctx = Self::emit_minus_skipped(src, node.range(), &edits);
    ...
    ///   inside it.
    /// * Any other `#scene(...)` call is subtracted whole.
    ...
    pub(crate) fn scene_context_preamble(&self, sid: usize) -> String {
        ...
        // The page owns its own background fill (default white in Typst). We do
        // NOT hardcode a fill here, so a user's `#set page(fill: …)` is honoured
        // and the canvas background is managed by the page, not by candy.
        preamble.push_str(&format!(
            "#set page(width: {pw_cm}cm, height: {doc_h_cm}cm, margin: 0pt)\n",
            pw_cm = pw_cm,
            doc_h_cm = doc_h_cm,
        ));
    ...
    pub(crate) fn synthesize_handbuilt_source(
        ...
        let (pw_cm, ph_cm) = scene
            ...
            .unwrap_or((16.0, 9.0));
        src.push_str(&format!(
            "#set page(width: {pw_cm}cm, height: {ph_cm}cm, margin: 0pt)\n"
        ));
        src.push_str("#scene()[\n");
