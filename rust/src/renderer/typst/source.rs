...
impl Renderer {
    ...
    ///   render the scene's Rust-injected `page()` (active == 0 or == its id),
    ///   emit just the scene *body* with **no** `page()` (so a nested descendant
    ///   scene inside
    ...
    pub(crate) fn build_parameterized_source(scene: &Scene) -> String {
        ...
        //    emit: its own Rust-injected `page()` (active == 0 or == its id), the
        //    *page* of the active descendant scene (so a nested child renders
        //    without a page-in-page — a `page()` inside another `page()` is
        //    illegal in Typst), or `none`. No weird parameters are added to the
        //    Typst `scene`
        ...
        for &sid in &order_inner {
            ...
            // The Typst `scene` is a placeholder; Rust injects the real page
            // (with the scene's canvas size + background) so the gating wrapper
            // emits a proper `page()` instead of a bare `scene()` marker.
            let page_call = Self::scene_page_call(scene, sid, call_inner);
            let mut wrapper = format!(
                "{{{{ let __a = sys.inputs.at(\"candy:active_scene\", default: 0); if __a == 0 or __a == {sid} {{ {page_call} }}"
            );
    ...
    /// Find the index of the bracket matching the one at `open` (`(` or `[`),
    /// accounting for nesting. Returns `None` if unbalanced.
    fn matching_close(s: &str, open: usize) -> Option<usize> {
        let open_c = *s.as_bytes().get(open)?;
        let (o, c) = match open_c {
            b'(' => (b'(', b')'),
            b'[' => (b'[', b']'),
            _ => return None,
        };
    ...
    /// Format a Typst length given in points as a clean `Ncm` / `N.Ncm` string
    /// (e.g. `453.543pt` → `16cm`, `255.118pt` → `9cm`).
    fn fmt_cm(pt: f64) -> String {
        let cm = pt / PT_PER_CM;
        if (cm.round() - cm).abs() < 1e-4 {
            return format!("{}cm", cm.round() as i64);
        }
        let s = format!("{cm:.4}cm");
        let trimmed = s.trim_end_matches('0').trim_end_matches('.');
        format!("{trimmed}cm")
    }

    /// Build the real `page(...)` call that Rust owns for a scene, from the
    /// scene's (Rust-known) `page_size` / `bg` and the body extracted from the
    /// original `#scene(...)` call. The Typst `scene` function is a pure
    /// placeholder, so the page is injected here rather than in `core.typ`.
    fn scene_page_call(scene: &Scene, sid: usize, call_inner: &str) -> String {
        // Extract the body content block from `scene(<args>)[<body>]`.
        let open = match call_inner.find('(') {
            Some(i) => i,
            None => return call_inner.to_string(),
        };
        let close = match Self::matching_close(call_inner, open) {
            Some(i) => i,
            None => return call_inner.to_string(),
        };
        let body_open = match call_inner[close..].find('[') {
            Some(i) => close + i,
            None => return call_inner.to_string(),
        };
        let body_close = match Self::matching_close(call_inner, body_open) {
            Some(i) => i,
            None => return call_inner.to_string(),
        };
        let body = &call_inner[body_open + 1..body_close];

        let info = scene.scenes.iter().find(|s| s.id == sid);
        let (w_pt, h_pt) = info
            .and_then(|s| s.page_size)
            .unwrap_or(crate::core::ast::DEFAULT_PAGE_PT);
        let fill = info
            .and_then(|s| s.bg.clone())
            .unwrap_or_else(|| "white".to_string());
        format!(
            "page(width: {}, height: {}, margin: 0pt, fill: {})[ {} ]",
            Self::fmt_cm(w_pt),
            Self::fmt_cm(h_pt),
            fill,
            body
    ...
    pub(crate) fn build_frame_inputs(
        ...
        for (label, st) in states {
            ...
            // currently playing. Every mobject is positioned by an out-of-flow
            // `place(top + left, …)`, so the whole scene compiles to a SINGLE
            // Typst page (out-of-flow content never overflows onto extra pages).
            // Page playback is therefore driven entirely by Rust: a mobject that
            // belongs to a non-active page is *hidden* (→ `none`), not merely
            // skipped — skipping would leave the wrapper at its defaults
            // (`hide = false`, `dx = dy = 0`) and draw the mobject at the page
            // origin on every page. Each on-page mobject uses its page-local
            // `nat`, so it lands correctly on the shared single-page canvas.
            // Mobjects without a recorded `page_of` (e.g. those absent from the
            // natural layout) are drawn on every page.
