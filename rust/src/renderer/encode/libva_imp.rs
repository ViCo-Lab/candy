...
// --- Local copies of the VA enums/constants we need -----------------------
// (kept here so the code reads clearly and does not depend on bindgen's
// double-prefixed constant names).
const VA_PROFILE_H264_HIGH: i32 = 7; // VAProfile_VAProfileH264High
const VA_PROFILE_HEVC_MAIN: i32 = 17; // VAProfile_VAProfileHEVCMain
const VA_PROFILE_AV1_0: i32 = 32; // VAProfile_VAProfileAV1Profile0
const VA_ENTRYPOINT_ENC_SLICE: u32 = 6; // VAEntrypoint_VAEntrypointEncSlice
const VA_CONFIG_ATTRIB_RT_FORMAT: u32 = 1;
const VA_CONFIG_ATTRIB_RATE_CONTROL: u32 = 2;
...
impl LibvaStream {
    ...
    pub fn new(
        ...
        let (profile, is_av1, is_hevc) = match codec {
            Codec::H264Libva => (VA_PROFILE_H264_HIGH, false, false),
            Codec::H265Libva => (VA_PROFILE_HEVC_MAIN, false, true),
            Codec::Av1Libva => (VA_PROFILE_AV1_0, true, false),
            _ => {
        ...
        let supports_encode = (0..num_ep as usize)
            .any(|i| entrypoints[i] == VA_ENTRYPOINT_ENC_SLICE as VAEntrypoint);
        if !supports_encode {
        ...
        va_ok(
            unsafe {
                vaCreateConfig(
                    dpy,
                    profile,
                    VA_ENTRYPOINT_ENC_SLICE as VAEntrypoint,
        ...
        let seq_buf = Self::create_seq_buffer(context, codec, w as u32, h as u32, fps)?;
        let rc_buf = Self::create_rc_buffer(context)?;
        let fr_buf = Self::create_framerate_buffer(context, fps)?;
    ...
    fn create_rc_buffer(context: VAContextID) -> Result<VABufferID, CandyError> {
    ...
    fn upload_nv12(&self, nv12: &[u8]) -> Result<(), CandyError> {
        ...
        va_ok(
            unsafe { vaCreateImage(self.dpy, &fmt, self.w as u16, self.h as u16, &mut image) },
            "vaCreateImage",
        )?;
    ...
    fn encode_nv12(&mut self, nv12: &[u8], is_idr: bool) -> Result<Vec<u8>, CandyError> {
        ...
        // Build the picture + slice parameter buffers (codec-specific).
    ...
    fn create_picture_buffer(
        ...
        let fi = self.frame_count as u16;
        let (data, size): (*mut c_void, u32) = if self.is_av1 {
            ...
            p.base_qindex = 128;
            p.frame_type_set_key_if_idr(&mut p, is_idr);
            (
    ...
    fn create_slice_buffer(&self, is_idr: bool) -> Result<VABufferID, CandyError> {
        ...
        let (data, size): (*mut c_void, u32) = if self.is_av1 {
            let mut s: VAEncSliceParameterBufferAV1 = unsafe { std::mem::zeroed() };
            s.slice_type = slice_type;
            (
                &s as *const _ as *mut c_void,
                std::mem::size_of::<VAEncSliceParameterBufferAV1>() as u32,
            )
        } else if self.is_hevc {
    ...
    pub fn push(&mut self, frame: &RenderedFrame) -> Result<(), CandyError> {
        ...
        let (sample, keyframe) = if self.is_av1 {
            let (s, priv_) = process_av1(&raw, &mut self.codec_private, !self.private_ready);
            (s, priv_.is_some() || self.private_ready == false && keyframe_of(raw))
        } else if self.is_hevc {
            let (s, priv_) = process_hevc(&raw, &mut self.codec_private, !self.private_ready);
            (s, priv_.is_some())
        } else {
            let (s, priv_) = process_h264(&raw, &mut self.codec_private, !self.private_ready);
            (s, priv_.is_some())
        };
        // `keyframe` above is only true when we extracted private data (frame 0).
        // Normalise: frame 0 is the keyframe; later frames are not.
        let is_key = if self.frame_count == 0 { true } else { false };
        let _ = keyframe;
...
/// Helper: recover the `VADisplay` associated with a context. VAAPI does not
/// expose a context→display lookup, so we stash the display on the stream and
/// reach it through the only place that needs it (buffer creation helpers that
/// take `context` but not `dpy`). We instead thread `self.dpy` everywhere those
/// helpers are called, so this is only used where the signature forces a
/// `context`-only call.
fn va_display_of(_context: VAContextID) -> VADisplay {
    // Unused placeholder; real calls pass `self.dpy`. Kept to satisfy the
    // buffer-creation helpers' `dpy` argument at the call sites.
    std::ptr::null_mut()
}
...
/// Heuristic: AV1 key frames begin with a Temporal Delimiter + Sequence Header
/// + Frame OBU; we treat frame 0 as the keyframe in the caller.
fn keyframe_of(_raw: &[u8]) -> bool {
    false
}
