...
mod imp {
    ...
    impl LibvaStream {
        ...
        pub fn new(
            ...
            let is_av1 = matches!(codec, Codec::Av1Libva);
            let is_hevc = matches!(codec, Codec::H265Libva);
            ...
            let mut minor: i32 = 0;
            va_ok(
                unsafe { vaInitialize(dpy, &mut major, &mut minor) },
                "vaInitialize",
            )?;
            ...
            va_ok(
                unsafe {
                    vaQueryConfigEntrypoints(dpy, profile, entrypoints.as_mut_ptr(), &mut num_ep)
                },
                "vaQueryConfigEntrypoints",
            )?;
            let supports_encode =
                (0..num_ep as usize).any(|i| entrypoints[i] == VA_ENTRYPOINT_ENC_SLICE);
            if !supports_encode {
            ...
            crate::info!("libva: direct VAAPI encode ({codec:?}) {w}x{h} @ {fps}fps (no ffmpeg)");
        ...
        fn create_seq_buffer(
            ...
            let (buf, size) = match codec {
                Codec::H264Libva => {
                    ...
                    s.seq_fields.bits.set_chroma_format_idc(1);
                    s.seq_fields.bits.set_frame_mbs_only_flag(1);
                ...
                Codec::Av1Libva => {
                    ...
                    s.ip_period = 1;
                    s.bits_per_second = 0;
                    s.seq_fields.bits.set_use_128x128_superblock(1);
                    (
                        &s as *const _ as *mut c_void,
        ...
        fn upload_nv12(&self, nv12: &[u8]) -> Result<(), CandyError> {
            ...
            va_ok(
                unsafe {
                    vaCreateImage(self.dpy, &fmt, self.w as c_int, self.h as c_int, &mut image)
                },
                "vaCreateImage",
            )?;
            ...
            va_ok(
                unsafe { vaUnmapBuffer(self.dpy, image.buf) },
                "vaUnmapBuffer(image)",
            )?;
            va_ok(
                unsafe {
            ...
            va_ok(
                unsafe { vaDestroyImage(self.dpy, image.image_id) },
                "vaDestroyImage",
            )?;
            Ok(())
        }
        ...
        fn encode_nv12(&mut self, nv12: &[u8], is_idr: bool) -> Result<Vec<u8>, CandyError> {
            ...
            va_ok(
                unsafe { vaEndPicture(self.dpy, self.context) },
                "vaEndPicture",
            )?;
            va_ok(
            ...
            while !seg.is_null() {
                ...
                if s.size > 0 && !s.buf.is_null() {
                    let bytes =
                        unsafe { std::slice::from_raw_parts(s.buf as *const u8, s.size as usize) };
                    out.extend_from_slice(bytes);
                }
                seg = s.next as *mut VACodedBufferSegment;
            }
            va_ok(
                unsafe { vaUnmapBuffer(self.dpy, coded_buf) },
                "vaUnmapBuffer(coded)",
            )?;
            va_ok(
                unsafe { vaDestroyBuffer(self.dpy, coded_buf) },
                "vaDestroyBuffer(coded)",
            )?;
            va_ok(
                unsafe { vaDestroyBuffer(self.dpy, pic_buf) },
                "vaDestroyBuffer(pic)",
            )?;
            va_ok(
                unsafe { vaDestroyBuffer(self.dpy, slice_buf) },
                "vaDestroyBuffer(slice)",
            )?;
        ...
        fn create_picture_buffer(
            ...
            let (data, size): (*mut c_void, u32) = if self.is_av1 {
                ...
                p.picture_flags
                    .bits
                    .set_frame_type(if is_idr { 0 } else { 1 });
                if is_idr {
                ...
                p.pic_fields
                    .bits
                    .set_idr_pic_flag(if is_idr { 1 } else { 0 });
                (
                    &p as *const _ as *mut c_void,
        ...
        fn create_slice_buffer(&self, is_idr: bool) -> Result<VABufferID, CandyError> {
            ...
            let mut id: VABufferID = VA_INVALID_ID;
            va_ok(
                unsafe { vaCreateBuffer(self.dpy, self.context, buf_type, size, 1, data, &mut id) },
        ...
        pub fn push(&mut self, frame: &RenderedFrame) -> Result<(), CandyError> {
            ...
            let sample = if self.is_av1 {
                ...
                process_av1_inner(&obus, &mut self.codec_private, first).0
            } else if self.is_hevc {
        ...
        pub fn finish(
            ...
            if self.codec_private.is_empty() {
                return Err(CandyError::Libva(
                    "libva encode produced no codec-private data (sequence header missing)".into(),
                ));
            }
            // Take ownership of samples file path before dropping the handle
            let samples_path = self.samples_path.clone();
            let sample_sizes = self.sample_sizes.clone();
            let keyframes = self.keyframes.clone();
            let codec_private = self.codec_private.clone();
...
impl LibvaStream {
    ...
    pub fn push(
        &mut self,
        _frame: &crate::renderer::RenderedFrame,
...
fn rgba_to_nv12(rgba: &[u8], w: usize, h: usize) -> Vec<u8> {
    ...
    let (y_plane, rest) = nv12.split_at_mut(wh);
    let uv_plane = &mut rest[0..wh / 2];
    for y in 0..h {
        for x in 0..w {
            let i = (y * w + x) * 4;
            let r = rgba[i] as f32;
...
fn process_av1(raw: &[u8], private: &mut Vec<u8>, first: bool) -> (Vec<u8>, Option<Vec<u8>>) {
    let obus = parse_av1_obus(raw);
    process_av1_inner(&obus, private, first)
}

/// Inner AV1 processing that takes pre-parsed OBUs (avoids borrowing `raw` mutably).
#[cfg(feature = "libva")]
fn process_av1_inner(
    obus: &[(u8, Vec<u8>)],
    private: &mut Vec<u8>,
    first: bool,
    ...
    for (t, payload) in obus {
        ...
        let header = (payload_first_header(*t) & !0x02) | 0x02;
        out.push(header);
        // LEB128 size
...
#[cfg(feature = "libva")]
fn payload_first_header(obu_type: u8) -> u8 {
