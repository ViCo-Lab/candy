...
| `--codec` | `h264` | `av1` / `h264` / `h265` / `x264` / `x265` / `h264-vaapi` / `h265-vaapi` / `h264-videotoolbox` / `h265-videotoolbox` / `h264-qsv` / `h265-qsv` / `h264-libva` / `h265-libva` / `av1-libva`. The first three are self-contained (rav1e/openh264); the `*-vaapi` / `*-videotoolbox` / `*-qsv` variants shell out to system ffmpeg (runtime-detected, no cargo dep); the `*-libva` variants are Linux-only direct VAAPI (no ffmpeg subprocess). See [Codecs](codecs.md). The hardware `*-vaapi` / `*-videotoolbox` / `*-qsv` / `*-libva` variants are conditionally compiled and appear in `--help` only on their native platform (VAAPI/libva → Linux, VideoToolbox → macOS, QSV → Windows). |
