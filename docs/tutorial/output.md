...
| `-o, --output` (repeatable) | — | Output name hint under `dist/` for videos; ignored for SVG drafts. A single-file (non-batch) build may pass a precise path here (it contains `/`, or `\` on Windows, or is the platform-independent link `.` / `..`) — it is written exactly there (with `.` / `..` resolved by the OS) instead of `dist/`, and `--output-dir` is ignored. |
| `-r, --recursive <dir>` (repeatable) | — | Recursively render every `.tyx` under the given directory(ies). Each argument must be a directory (a file or missing path is a fatal E001); hidden directories (`.git`, `.candy`, …) are skipped. The source tree is mirrored inside a folder named after the source directory under `--output-dir` (`<root>/a/b.tyx` → `<output-dir>/<root-name>/a/b.<ext>`, root-level `<root>/root.tyx` → `<output-dir>/<root-name>/root.<ext>`). Combine with explicit `<input>` files and repeat freely. |
